...
pairs = [
    ...
    [NaiveBayes(labels, attributes_scheme['nominal']), RBF()],
    ...
    [Perceptron(labels, attributes_scheme['numeric']), RBF()],
    ...
    [HoeffdingTree(labels, attributes_scheme['nominal']), RBF()],
]

# 4. Creating a color set for plotting results
colors = [
    Color.Indigo[1], Color.Blue[1], Color.Green[1], Color.Lime[1], Color.Yellow[1],
...
prequential = PrequentialMultiPairs(pairs, attributes, attributes_scheme,
                                    ...
                                    w_vec, project, color_set=colors, legend_param=True)
